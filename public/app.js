...
var addUserOnServer=function(firstname,lastname,email,password){
    ...
    return fetch("https://plant-nursery.herokuapp.com/users", {
        body:data,
        method:"POST",
        credentials:"include",
        headers:{
...
var postToSession=function(email,password){
    var data= `email=${encodeURIComponent(email)}`;
    data+= `&plainPassword=${encodeURIComponent(password)}`;
    return fetch("https://plant-nursery.herokuapp.com/session", {
        body:data,
        method:"POST",
        credentials:"include",
        headers:{
...
var logoutFromServer=function(logout){
    return fetch("https://plant-nursery.herokuapp.com/session" ,{
        method:"DELETE",
        credentials:"include",
...
var app = new Vue({
    ...
    methods:{
        ...
        SignUpButtonClicked:function(){
            addUserOnServer(this.fname,this.lname,this.email,this.pass).then((response)=>{
            this.added=[];
            
            if(response.status==201){
                this.added.push("Successfully Signed Up");
                this.fname="",
                this.lname="",
                this.email="",
                this.pass=""
            ...
            if(response.status==500){
                this.added.push("Email or password already in use");
            }
            if(response.status==422){
                this.added.push("Please Enter all fields");
            }
            });
        },
        LogInButton:function(){
            postToSession(this.emailS,this.passw).then((response)=>{
            this.loginmessage=[];
            if(response.status==201){
                this.emailS="";
                this.passw="";
                ...
                this.loginmessage.push("Incorrect email or password");
            }
            });
        },
